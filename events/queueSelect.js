    ...
    async execute(bot, interaction) {
        ...
        switch (cidData[1]) {
            case "submitregister": {
                ...
                const update = await bot.database("UPDATE queue SET user = " + userCache.id + " WHERE type = '" + GetReverseQueueNameMapping(cidData[2]) + "' AND date = '" + formatDate + "'")
            ...
            case "submitunregister": {
                ...
                const update = await bot.database("UPDATE queue SET user = NULL WHERE type = '" + GetReverseQueueNameMapping(cidData[2]) + "' AND date = '" + formatDate + "'")
                if (!update) return interaction.editReply({ embeds: [bot.prettyReply("**Wystąpił błąd, spróbuj ponownie później**", interaction)], ephemeral: true })
